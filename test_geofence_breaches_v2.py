...
def test_geofence_breach_detection():
    ...
    print(f"   ✓ User created: ID={user_id}, token={access_token[:20]}...")
    
    # 2. Register device
    print("\n2. Registering test device...")
    response = requests.post(f"{BASE_URL}/v1/registerDevice", json=TEST_DEVICE)
    if response.status_code != 200:
        print(f"   ERROR: {response.status_code} {response.text}")
        return
    ...
    response = requests.post(
        ...
        json=gps_data,
        headers={"Access-Token": TEST_DEVICE['access_token']}
    )
    if response.status_code != 200:
        print(f"   ERROR: {response.status_code} {response.text}")
        return
    ...
    entered_events = [e for e in events if e.get('event_type') == 'ENTERED']
    if len(entered_events) == 0:
        print(f"   WARNING: No ENTERED event found. Events: {events}")
    else:
        print(f"   ✓ ENTERED event detected: {entered_events[0].get('event_id', 'unknown')}")
    ...
    if response.status_code != 200:
        ...
        return
    events = response.json()
    exited_events = [e for e in events if e.get('event_type') == 'EXITED']
    if len(exited_events) == 0:
        print(f"   WARNING: No EXITED event found. Events: {events}")
    else:
        print(f"   ✓ EXITED event detected: {exited_events[0].get('event_id', 'unknown')}")
    ...
    events = response.json()
    entered_events = [e for e in events if e.get('event_type') == 'ENTERED']
    print(f"   ✓ Total ENTERED events: {len(entered_events)}")
    
    print("\n=== TESTS COMPLETED ===\n")
    print(f"Summary:")
    print(f"  - User: {TEST_USER['email_address']} (ID: {user_id})")
    ...
    print(f"  - ENTERED events: {len([e for e in events if e.get('event_type') == 'ENTERED'])}")
    print(f"  - EXITED events: {len([e for e in events if e.get('event_type') == 'EXITED'])}")
